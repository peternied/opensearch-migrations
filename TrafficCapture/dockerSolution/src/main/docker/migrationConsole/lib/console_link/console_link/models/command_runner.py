...
class CommandRunner:
    ...
    def sanitized_command(self) -> List[str]:
        ...
        for field in self.sensitive_fields:
            if field in display_command:
                ...
                if (
                    len(display_command) > (field_index + 1)
                    and display_command[field_index + 1] == self.command_args[field]
                ):
    ...
    def _run_as_synchronous_process(self) -> CommandResult:
        try:
            ...
            subprocess.run(
                self.command, stdout=None, stderr=None, text=True, check=True
            )
            return CommandResult(success=True, value="Command executed successfully")
        except subprocess.CalledProcessError as e:
            raise CommandRunnerError(
                e.returncode, self.sanitized_command(), e.stderr, self
            )
        ...
        try:
            with open(log_file, "w") as f:
                ...
                return CommandResult(
                    success=True,
                    value=f"Process started with PID {process.pid}\n"
