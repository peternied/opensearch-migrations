...
def handle_errors(
    service_type: str,
    on_success: Callable[[Any], Tuple[ExitCode, str]] = lambda status: (
        ExitCode.SUCCESS,
        status,
    ),
    on_failure: Callable[[Any], Tuple[ExitCode, str]] = lambda status: (
        ExitCode.FAILURE,
        status,
    ),
) -> Callable[[Any], Tuple[ExitCode, str]]:
    def decorator(
        func: Callable[[Any], Tuple[ExitCode, str]]
    ) -> Callable[[Any], Tuple[ExitCode, str]]:
        def wrapper(service: Service, *args, **kwargs) -> Tuple[ExitCode, str]:
            try:
                ...
                return (
                    ExitCode.FAILURE,
                    f"Failure on {func.__name__} for {service_type}: {type(e).__name__} {e}",
                )
            if result.success:
