...
class Cluster:
    ...
    def __init__(
        ...
        self.allow_insecure = (
            config.get("allow_insecure", False)
            if self.endpoint.startswith("https")
            else config.get("allow_insecure", True)
        )
    ...
    def get_basic_auth_password(self) -> str:
        ...
        client = create_boto3_client(
            aws_service_name="secretsmanager", client_options=self.client_options
        )
        password = client.get_secret_value(
            SecretId=self.auth_details["password_from_secret_arn"]
        )
    ...
    def _get_sigv4_details(self, force_region=False) -> tuple[str, Optional[str]]:
        ...
        if force_region and "region" not in self.auth_details:
            ...
            return self.auth_details.get("service", "es"), self.auth_details.get(
    ...
    def _generate_auth_object(self) -> requests.auth.AuthBase | None:
        if self.auth_type == AuthMethod.BASIC_AUTH:
            ...
            return HTTPBasicAuth(self.auth_details.get("username", None), password)
    ...
    def call_api(
        ...
        **kwargs,
        ...
        if self.client_options and self.client_options.user_agent_extra:
            ...
            request_headers = append_user_agent_header_for_requests(
                headers=headers, user_agent_extra=user_agent_extra
            )
        ...
        logger.info(
            f"Received response: {r.status_code} {method.name} {self.endpoint}{path} - {r.text[:1000]}"
        )
        if raise_error:
            r.raise_for_status()
        return r

    def execute_benchmark_workload(
        self,
        workload: str,
        ...
        if self.auth_type == AuthMethod.BASIC_AUTH:
            ...
            client_options += (
                f",basic_auth_user:{self.auth_details['username']},"
                f"basic_auth_password:{password_to_censor}"
            )
        elif self.auth_type == AuthMethod.SIGV4:
            raise NotImplementedError(
                f"Auth type {self.auth_type} is not currently support for executing "
        ...
        command = (
            f"opensearch-benchmark execute-test --distribution-version=1.0.0 --target-host={self.endpoint} "
        ...
        display_command = command.replace(
            f"basic_auth_password:{password_to_censor}", "basic_auth_password:********"
        )
        logger.info(f"Executing command: {display_command}")
        subprocess.run(command, shell=True)

    def fetch_all_documents(
        self, index_name: str, batch_size: int = 100
