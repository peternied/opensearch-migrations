...
def raise_for_aws_api_error(response: Dict) -> None:
    ...
    if status_code not in range(200, 300):
        raise AWSAPIError("Non-2XX status code received", status_code=status_code)
...
def append_user_agent_header_for_requests(
    headers: Optional[dict], user_agent_extra: str
):
    adjusted_headers = dict(headers) if headers else {}
    if "User-Agent" in adjusted_headers:
        adjusted_headers["User-Agent"] = (
            f"{adjusted_headers['User-Agent']} {user_agent_extra}"
        )
    else:
        adjusted_headers["User-Agent"] = (
            f"{requests.utils.default_user_agent()} {user_agent_extra}"
        )
...
class SigV4AuthPlugin(requests.auth.AuthBase):
    ...
    def __call__(self, r: PreparedRequest) -> PreparedRequest:
        ...
        aws_request = AWSRequest(
            method=r.method, url=r.url, data=r.body, headers=filtered_headers
        )
