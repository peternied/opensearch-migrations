...
class DockerReplayer(Replayer):
    ...
    def start(self, *args, **kwargs) -> CommandResult:
        ...
        return CommandResult(
            success=True, value="No action performed, action is unimplemented"
        )
    ...
    def get_status(self, *args, **kwargs) -> CommandResult:
        logger.warning(
            "Get status command is not implemented for Docker Replayer and "
            "always assumes service is running"
        )
        return CommandResult(
            True, (ReplayStatus.RUNNING, "Docker Replayer is assumed to be running")
        )
