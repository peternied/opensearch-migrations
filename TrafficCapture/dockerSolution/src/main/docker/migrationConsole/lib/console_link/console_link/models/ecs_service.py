...
class ECSService:
    def __init__(
        self, cluster_name, service_name, aws_region=None, client_options=None
    ):
    ...
    def set_desired_count(self, desired_count: int) -> CommandResult:
        logger.info(
            f"Setting desired count for service {self.service_name} to {desired_count}"
        )
        ...
        return CommandResult(
            True,
            f"Service {self.service_name} set to {desired_count} desired count."
