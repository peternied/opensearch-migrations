...
from console_link.models.metrics_source import (
...
BackfillType = Enum("BackfillType", ["opensearch_ingestion", "reindex_from_snapshot"])
...
def get_backfill(
    config: Dict,
    source_cluster: Optional[Cluster],
    target_cluster: Optional[Cluster],
    client_options: Optional[ClientOptions] = None,
) -> Backfill:
    if BackfillType.opensearch_ingestion.name in config:
        if source_cluster is None:
            raise ValueError(
                "source_cluster must be provided for OpenSearch Ingestion backfill"
            )
        if target_cluster is None:
            raise ValueError(
                "target_cluster must be provided for OpenSearch Ingestion backfill"
            )
        logger.debug("Creating OpenSearch Ingestion backfill instance")
        return OpenSearchIngestionBackfill(
            config=config,
...
def get_metrics_source(config, client_options: Optional[ClientOptions] = None):
    if "prometheus" in config:
        ...
        logger.error(
            f"An unsupported metrics source type was provided: {config.keys()}"
        )
