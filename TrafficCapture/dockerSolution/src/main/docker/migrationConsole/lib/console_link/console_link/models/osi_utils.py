...
def sanitize_endpoint(endpoint: str, remove_port: bool):
    ...
    if scheme is None or hostname is None:
        raise RuntimeError(
            f"Provided endpoint does not have proper endpoint formatting: {endpoint}"
        )
    ...
    if regex_list is not None:
        for regex_string in regex_list:
            try:
                ...
                logger.error(
                    f"Unable to compile provided index inclusion regex string: {regex_string}"
                )
                raise e
...
def generate_source_index_config(include_index_regex_list=None):
    if include_index_regex_list is not None:
        ...
        for regex_str in include_index_regex_list:
            include_template_str = (
                include_template_str + f"          - index_name_regex: {regex_str}\n"
            )
...
def validate_pipeline_config_arguments(
    source_auth_type: AuthMethod,
    target_auth_type: AuthMethod,
    source_auth_secret=None,
    pipeline_role_arn=None,
    include_index_regex_list=None,
    aws_region=None,
):
    # Validation of auth options provided
    if aws_region is None and (
        source_auth_type == AuthMethod.SIGV4 or target_auth_type == AuthMethod.SIGV4
    ):
        raise InvalidAuthParameters(
            "AWS region must be provided for a source or target auth type of SIGV4"
        )

    if pipeline_role_arn is None and source_auth_type == AuthMethod.SIGV4:
        raise InvalidAuthParameters(
            "Source pipeline role ARN must be provided for an auth type of SIGV4"
        )

    if pipeline_role_arn is None and target_auth_type == AuthMethod.SIGV4:
        raise InvalidAuthParameters(
            "Target pipeline role ARN must be provided for an auth type of SIGV4"
        )

    if (
        source_auth_secret is None or pipeline_role_arn is None
    ) and source_auth_type == AuthMethod.BASIC_AUTH:
        raise InvalidAuthParameters(
            "Source auth secret and pipeline role ARN to access secret, must be provided "
            "for an auth type of BASIC_AUTH"
        )
    validate_index_regex_list(include_index_regex_list)


def construct_pipeline_config(
    ...
    validate_pipeline_config_arguments(
    ...
    pipeline_config = pipeline_config.replace(
        INDEX_SELECTION_OPTIONS_PLACEHOLDER, index_config
    )

    # Fill in OSI pipeline template file from provided options
    if source_auth_type == AuthMethod.BASIC_AUTH:
        secret_config = generate_source_secret_config(
            source_auth_secret, pipeline_role_arn, aws_region
        )
        pipeline_config = pipeline_config.replace(
            AWS_SECRET_CONFIG_PLACEHOLDER, secret_config
        )
        pipeline_config = pipeline_config.replace(
            SOURCE_AUTH_OPTIONS_PLACEHOLDER, SOURCE_BASIC_AUTH_CONFIG_TEMPLATE.strip()
        )
    else:
        pipeline_config = pipeline_config.replace(AWS_SECRET_CONFIG_PLACEHOLDER, "")

    if source_auth_type == AuthMethod.SIGV4:
        aws_source_config = generate_source_sigv4_auth_config(
            pipeline_role_arn, aws_region
        )
        pipeline_config = pipeline_config.replace(
            SOURCE_AUTH_OPTIONS_PLACEHOLDER, aws_source_config
        )

    if target_auth_type == AuthMethod.SIGV4:
        aws_target_config = generate_target_sigv4_auth_config(
            pipeline_role_arn, aws_region
        )
        pipeline_config = pipeline_config.replace(
            TARGET_AUTH_OPTIONS_PLACEHOLDER, aws_target_config
        )

    pipeline_config = pipeline_config.replace(
        SOURCE_ENDPOINT_PLACEHOLDER, source_endpoint
    )
    pipeline_config = pipeline_config.replace(
        TARGET_ENDPOINT_PLACEHOLDER, target_endpoint
    )
...
def get_status(osi_client, pipeline_name: str):
    ...
    get_pipeline_response = osi_client.get_pipeline(PipelineName=name)

    return {
        "status": get_pipeline_response["Pipeline"]["Status"],
        "statusMessage": get_pipeline_response["Pipeline"]["StatusReason"][
            "Description"
        ],
    }
...
def create_pipeline(
    ...
    cw_log_options = {"IsLoggingEnabled": False}
    ...
    if cw_log_group_name is not None:
        cw_log_options = {
            ...
            "CloudWatchLogDestination": {"LogGroup": cw_log_group_name},
        }
    ...
    osi_client.create_pipeline(
        ...
        VpcOptions={"SubnetIds": subnet_ids, "SecurityGroupIds": security_group_ids},
...
def create_pipeline_from_env(
    ...
    source_endpoint_clean = sanitize_endpoint(
        endpoint=source_cluster.endpoint, remove_port=False
    )
    ...
    if (
        ...
        and "password_from_secret_arn" in source_cluster.auth_details
    ):
        source_auth_secret = source_cluster.auth_details["password_from_secret_arn"]
    pipeline_config_string = construct_pipeline_config(
...
def create_pipeline_from_json(
    osi_client, input_json: Dict, pipeline_template_path: str
):
    ...
    source_endpoint_clean = sanitize_endpoint(
        endpoint=source_provider.get("Uri"), remove_port=remove_source_port
    )
