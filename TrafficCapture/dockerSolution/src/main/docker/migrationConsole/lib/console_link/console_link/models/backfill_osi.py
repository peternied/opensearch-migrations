...
from console_link.models.osi_utils import (
...
OSI_SCHEMA = {
    "pipeline_role_arn": {"type": "string", "required": True},
    "vpc_subnet_ids": {
        "type": "list",
        "required": True,
        "schema": {
    ...
    "log_group_name": {"type": "string", "required": False},
    "tags": {
        "type": "list",
        "required": False,
        "schema": {
...
class OpenSearchIngestionBackfill(Backfill):
    ...
    def __init__(
        ...
        if not v.validate(config):
            raise ValueError(
                "Invalid config file for OpenSearchIngestion migration", v.errors
            )
        self.osi_props = OpenSearchIngestionMigrationProps(config=config)
        self.osi_client = create_boto3_client(
            aws_service_name="osis", client_options=self.client_options
        )
    ...
    def create(
        self,
        pipeline_template_path="/root/osiPipelineTemplate.yaml",
        print_config_only=False,
    ):
        create_pipeline_from_env(
            osi_client=self.osi_client,
