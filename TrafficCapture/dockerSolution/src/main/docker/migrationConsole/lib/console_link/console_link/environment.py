...
class Environment:
    ...
    def __init__(self, config_file: str):
        ...
        if "client_options" in self.config:
            self.client_options: ClientOptions = ClientOptions(
                self.config["client_options"]
            )

        if "source_cluster" in self.config:
            self.source_cluster = Cluster(
                config=self.config["source_cluster"], client_options=self.client_options
            )
        ...
        if "target_cluster" in self.config:
            self.target_cluster: Cluster = Cluster(
                config=self.config["target_cluster"], client_options=self.client_options
            )
            logger.info(f"Target cluster initialized: {self.target_cluster.endpoint}")
        else:
            logger.warning(
                "No target cluster provided. This may prevent other actions from proceeding."
            )

        if "metrics_source" in self.config:
            self.metrics_source: MetricsSource = get_metrics_source(
                config=self.config["metrics_source"], client_options=self.client_options
            )
        ...
        if "backfill" in self.config:
            self.backfill: Backfill = get_backfill(
        ...
        if "replay" in self.config:
            self.replay: Replayer = get_replayer(
                self.config["replay"], client_options=self.client_options
            )
            logger.info(f"Replay initialized: {self.replay}")

        if "snapshot" in self.config:
            self.snapshot: Snapshot = get_snapshot(
                self.config["snapshot"], source_cluster=self.source_cluster
            )
        ...
        if "metadata_migration" in self.config:
            self.metadata: Metadata = Metadata(
                self.config["metadata_migration"],
                target_cluster=self.target_cluster,
