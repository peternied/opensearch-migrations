...
import { MigrationSession, SessionWorkflow, useMigrationSessions } from '@/context/migration-session';
...
const WORKFLOW_OPTIONS: WorkflowOption[] = [
  {
    value: 'backfill',
    label: 'Backfill',
    description: 'Transfer existing historical data to the new system without impacting live traffic.',
    iconName: 'upload'
  },
  {
    value: 'replay',
    label: 'Traffic Capture/Replay',
    description: 'Capture live traffic and replay it in the new environment for testing and validation.',
    iconName: 'redo'
  },
  {
    value: 'full',
    label: 'Combined Capture + Replay',
    description: 'Perform backfill and then begin traffic capture and replay in a single workflow.',
    iconName: 'command-prompt'
  },
  {
    value: 'freeform',
    label: 'Freeform Exploration',
    description: 'Explore the Migration Assistant website without committing to a specific workflow.',
    iconName: 'star'
  }
];
      ...
      <Container>
        <SpaceBetween size="l">
          ...
          <FormField label="Workflow Type" description="Choose the appropriate workflow.">
            <Tiles
              value={workflow}
              onChange={({ detail }) => setWorkflow(detail.value as SessionWorkflow)}
          ...
          <SpaceBetween size='m' direction='horizontal'>
            <Button variant="primary" disabled={!name} disabledReason='Name is required' onClick={handleAddSession}>Create Session</Button>
