...
import { Box, Header, SpaceBetween, Wizard, WizardProps } from '@cloudscape-design/components';
...
function StepPageContent() {
  ...
  const [activeStepIndex, setActiveStepIndex] = useState(
    isNaN(initialStep) ? 0 : Math.min(Math.max(initialStep, 0), steps.length - 1)
  );

  const handleStepChange = ({ detail }: { detail: { requestedStepIndex: number } }) => {
